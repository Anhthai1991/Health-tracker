        ...
        async function callGemini(prompt) {
            const key = localStorage.getItem('geminiKey');
            if(!key) { alert('Nhập API Key!'); return null; }
            ...
            if (!finalResult) alert('AI Error. Kiểm tra Key/Quota.');
            return finalResult;
        }
...
// 1. Hàm chuyển đổi giao diện
function toggleView(view) {
    const main = document.querySelector('.main-content > .row:not(#workoutSection)'); // Chọn các row cũ
    const woSection = document.getElementById('workoutSection');
    
    // Ẩn hiện các phần tử Dashboard cũ
    const rows = document.querySelectorAll('.main-content > .row');
    
    if (view === 'workout') {
        rows.forEach(r => { if(r.id !== 'workoutSection') r.style.display = 'none'; });
        woSection.style.display = 'block';
        renderWorkouts();
        if(workoutLogs.length === 0) askAIWorkout(); // Lần đầu vào thì hỏi AI luôn
    } else {
        rows.forEach(r => { if(r.id !== 'workoutSection') r.style.display = 'flex'; }); // Hoặc block tùy layout cũ
        // Fix nhanh hiển thị lại dashboard
        location.reload(); // Cách đơn giản nhất để reset lại view Dashboard chuẩn
    }
}

// 2. Hàm tính Calo ước tính (Tương đối)
function estimateBurn(type, mins, weight) {
    ...
    // Công thức: Kcal = MET * Weight(kg) * (Time(min)/60)
...
function addWorkoutLog() {
    ...
    // Lưu vào LocalStorage (Vì ta không sửa được file data.json trực tiếp)
...
function renderWorkouts() {
    let storedLogs = JSON.parse(localStorage.getItem('workoutLogs') || '[]');
    workoutLogs = storedLogs; // Sync global var
    ...
    // Cập nhật thống kê
    document.getElementById('woCount').innerText = storedLogs.length;
    const totalBurn = storedLogs.reduce((sum, l) => sum + l.calories, 0);
    document.getElementById('woBurn').innerText = totalBurn;
}

// 5. Hàm AI Gợi ý bài tập (Dùng key cũ)
async function askAIWorkout() {
    document.getElementById('aiWorkoutTip').innerHTML = '<div class="spinner-border spinner-border-sm"></div> Đang suy nghĩ...';
    ...
    const res = await callGemini(prompt);
    if(res) document.getElementById('aiWorkoutTip').innerText = res;
}
